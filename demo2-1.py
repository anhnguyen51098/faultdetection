...
#### NEWCODE 23/2/2022
def canny_edge_detection_for_gray(gray, blur_ksize=5, threshold1=100, threshold2=250):
    """
    image_path: link to image
    ...
    img_canny = cv2.Canny(img_gaussian, threshold1, threshold2, L2gradient=True)
...
def check_crack(img):
    ### input gray image
    # extract features
    _, thresh = cv2.threshold(img, 120, 255, cv2.THRESH_TOZERO)
    gray_x = thresh.astype(np.uint8)
    mask = np.zeros(thresh.shape, np.uint8)
    _, thresh2 = cv2.threshold(gray_x, 100, 255, 0)

    # count object
    img_canny = canny_edge_detection_for_gray(thresh2, 3, 100, 150)
    gray_x2 = img_canny.astype(np.uint8)
    mask = np.zeros(img_canny.shape, np.uint8)
    _, thresh3 = cv2.threshold(gray_x2, 200, 255, 0)

    contours, _ = cv2.findContours(gray_x2, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    count = 0
    for cnt in contours:
    ...
    #cv2.imwrite('obj_crack.jpg', mask)
    #st.image(mask, caption="crack detect")
...
if input_image is not None:
    ...
    for obj in data_object_dict:
        ...
        if (sum(map(lambda x: x[4] == 2, data_object_dict[obj]))) == 0:
            st.write('Object ' + str(obj + 1) + ' has no shape defect')
        else:
            st.write('Object ' + str(obj + 1) + ' has shape defect')
        ...
        try:
            ...
            if len(right_list) > 3:
                st.write('Object has black artifact in space')
                right_list_y.pop(1)
                print(right_list_y)
                right_list.pop(1)
                print(right_list)
                ...
                print(A - A2)
                if (A - A2) < 6:
                    st.write('Object has no black artifact in cap')
                else:
                    st.write('Object has black artifact in cap')

            else:
                st.write('Object has no black artifact in space')
                D = max(bottom_list_x) - min(top_list_x)
                B = max(right_list_y) - min(right_list_y)
                A = right_list_y[1] - min(right_list_y)
                A2 = left_list_y[1] - min(right_list_y)
            ...
            #### NEWCODE CHECK CRACK
            if (check_crack(img_add_bg)):
                st.write('Object has crack')
            else:
                st.write('Object has no crack')

            ### NEWCODE CHECK WHITE (BUBBLE)
            if (check_white(img_final2)):
                st.write('Object has white details (bubbles)')
            else:
                st.write('Object has no white details (bubbles)')
